...
namespace sr::crypto
{
    ...
    inline const std::byte* as_bytes(const auto& arr) { return reinterpret_cast<const std::byte*>(arr.data()); }

    inline std::byte* as_bytes(auto& arr) { return reinterpret_cast<std::byte*>(arr.data()); }

    inline const unsigned char* as_uchar(const auto& arr) { return reinterpret_cast<const unsigned char*>(arr.data()); }

    inline unsigned char* as_uchar(auto& arr) { return reinterpret_cast<unsigned char*>(arr.data()); }
