...
namespace sr::crypto
{
    ...
    std::vector<std::byte> seal(std::span<const std::byte> plaintext, const Ed25519PubKey& recipient_pk);
    ...
    std::optional<std::vector<std::byte>> unseal(
        std::span<const std::byte> ciphertext, const Ed25519PubKey& our_pk, const Ed25519SecKey& our_sk);
