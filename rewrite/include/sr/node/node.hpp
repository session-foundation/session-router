...
private:
    ...
    struct SessionTagHash {
        size_t operator()(const sr::session::SessionTag& t) const {
            size_t h = 0;
            for (auto b : t) h = h * 31 + static_cast<size_t>(b);
            return h;
        }
    };
    ...
    void handle_path_build(const sr::contact::RouterID& from,
                          std::span<const std::byte> payload,
                          std::function<void(std::vector<std::byte>)> respond);
    void handle_gossip_rc(const sr::contact::RouterID& from,
                         std::span<const std::byte> payload,
                         std::function<void(std::vector<std::byte>)> respond);
    void handle_datagram(const sr::contact::RouterID& from,
