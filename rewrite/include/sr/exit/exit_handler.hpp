...
public:
    ...
    bool enable(const std::string& exit_interface,
    ...
    void expire_nat_entries(std::chrono::steady_clock::time_point now,
                           std::chrono::seconds max_idle = std::chrono::minutes(5));
...
public:
    ...
    bool setup_exit_routes(const std::string& tun_name,
    ...
    bool setup_nat_rules(const std::string& tun_name,
                        const std::string& exit_interface);
