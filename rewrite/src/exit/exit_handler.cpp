...
bool ExitHandler::enable(const std::string& exit_interface,
...
void ExitHandler::handle_exit_packet(
    const sr::contact::RouterID& client,
    std::span<const std::byte> ip_packet) {
    if (!_enabled)
        return;
...
void ExitHandler::on_send_back(SendCallback cb) {
    _send_back = std::move(cb);
}

void ExitHandler::expire_nat_entries(
...
size_t ExitHandler::nat_table_size() const {
    return _nat_table.size();
}
...
std::optional<std::pair<sr::contact::RouterID, std::vector<std::byte>>>
ExitHandler::reverse_nat(std::span<const std::byte> packet) {
    if (packet.size() < 20)
        return std::nullopt;
...
bool RouteManager::setup_exit_routes(
    const std::string& tun_name,
    const std::string& exit_interface) {
    _tun_name = tun_name;
...
bool RouteManager::setup_nat_rules(
    const std::string& tun_name,
    const std::string& exit_interface) {
    ...
    if (system(cmd2.c_str()) != 0) return false;
    if (system(cmd3.c_str()) != 0) return false;
