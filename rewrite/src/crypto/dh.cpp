...
namespace sr::crypto
{
        ...
        const Nonce& nonce)
    {
        ...
        crypto_generichash_init(&state, as_uchar(nonce), nonce.size(), HASH_SIZE);
    ...
    Bytes<SHORT_HASH_SIZE> short_hash(std::span<const std::byte> data)
    {
        ...
        unsigned char key[crypto_shorthash_KEYBYTES] = {};
        crypto_shorthash(as_uchar(out), reinterpret_cast<const unsigned char*>(data.data()), data.size(), key);
        return out;
    }

    Bytes<HASH_SIZE> blake2b(std::span<const std::byte> data, std::span<const std::byte> key)
    {
        Bytes<HASH_SIZE> out;
        crypto_generichash(
            as_uchar(out),
