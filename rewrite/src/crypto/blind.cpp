...
namespace sr::crypto
{
    ...
    static Bytes<32> blinding_scalar(const Ed25519PubKey& pk, std::string_view domain)
    {
        Bytes<64> h64;
        crypto_generichash(
            as_uchar(h64),
    ...
    Ed25519PubKey blind_pubkey(const Ed25519PubKey& root_pk, std::string_view domain)
    {
        ...
        if (crypto_scalarmult_ed25519_noclamp(as_uchar(blinded), as_uchar(scalar), as_uchar(root_pk)) != 0)
        ...
        const Ed25519SecKey& root_sk, const Ed25519PubKey& root_pk, std::string_view domain)
    {
        ...
        if (crypto_scalarmult_ed25519_noclamp(as_uchar(bkp.pk), as_uchar(scalar), as_uchar(root_pk)) != 0)
        ...
        std::memcpy(reinterpret_cast<unsigned char*>(bkp.sign_key.data()) + 32, as_uchar(bkp.pk), 32);

        // hash_data = blake2b(root_sk_seed, key=domain)
        // Used as the nonce component in the Ed25519 signing
        crypto_generichash(
            as_uchar(bkp.hash_data),
            32,
            as_uchar(root_sk),
            32,  // just the seed portion
            reinterpret_cast<const unsigned char*>(domain.data()),
            domain.size());
    ...
    Signature BlindedKeyPair::sign(std::span<const std::byte> message) const
    {
        ...
        crypto_hash_sha512_update(&nh_state, reinterpret_cast<const unsigned char*>(message.data()), message.size());
        ...
        crypto_hash_sha512_update(&hram_state, reinterpret_cast<const unsigned char*>(message.data()), message.size());
        ...
        crypto_core_ed25519_scalar_mul(tmp, hram_scalar, reinterpret_cast<const unsigned char*>(sign_key.data()));
