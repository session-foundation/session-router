...
namespace sr::crypto
{
    ...
    std::span<std::byte> aead_encrypt_inplace(
        std::span<std::byte> buf, size_t plaintext_len, const SymmetricKey& key, const Nonce& nonce)
    {
    ...
    std::optional<std::span<std::byte>> aead_decrypt_inplace(
        std::span<std::byte> buf, const SymmetricKey& key, const Nonce& nonce)
    {
    ...
    std::vector<std::byte> aead_encrypt(
        std::span<const std::byte> plaintext, const SymmetricKey& key, const Nonce& nonce)
    {
    ...
    std::optional<std::vector<std::byte>> aead_decrypt(
        std::span<const std::byte> ciphertext, const SymmetricKey& key, const Nonce& nonce)
    {
    ...
    void xchacha20_inplace(std::span<std::byte> buf, const SymmetricKey& key, const Nonce& nonce)
    {
