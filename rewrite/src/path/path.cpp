...
std::vector<std::byte> Path::encrypt_data(
    ...
    const Nonce& nonce) const
{
    ...
    for (int i = static_cast<int>(_hops.size()) - 1; i >= 0; --i) {
        xchacha20_inplace(buf,
            reinterpret_cast<const SymmetricKey&>(_hops[i].shared_secret),
            hop_nonces[i]);
    }
