...
namespace sr::path
{
        ...
        std::chrono::seconds lifetime)
    {
        ...
        for (int i = static_cast<int>(n_hops) - 1; i >= 0; --i)
        {
            ...
            std::memcpy(
                frame_start + 56, encrypted_payload.data(), std::min(sizeof(encrypted_payload), BUILD_FRAME_SIZE - 56));
            ...
            {
                auto other_frame = std::span<std::byte>(result.frames.data() + j * BUILD_FRAME_SIZE, BUILD_FRAME_SIZE);
