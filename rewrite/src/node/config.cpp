    ...
    Config Config::from_file(const std::filesystem::path& path)
    {
        ...
        while (std::getline(file, line))
        {
            ...
            if (section == "router")
            {
                if (key == "data-dir")
                    cfg.data_dir = val;
                else if (key == "is-relay")
                    cfg.is_relay = (val == "true" || val == "1");
            }
            else if (section == "network")
            {
                if (key == "listen")
                    ...
                    cfg.listen_port = static_cast<uint16_t>(std::stoi(val));
            }
            else if (section == "tun")
            {
                if (key == "name")
            ...
            {
                if (key == "hops")
                    cfg.min_path_hops = std::stoi(val);
                else if (key == "target")
                    cfg.target_paths = std::stoi(val);
            }
            else if (section == "bootstrap")
            {
                if (key == "file")
                    cfg.bootstrap_files.push_back(val);
            }
        }
