    ...
    , _manager{_endpoint}
{
    ...
    _manager.on_datagram([this](auto& from, auto data) {
        handle_datagram(from, data);
    });
...
void Node::run() {
    ...
    // Start listening
    auto seed = std::span<const std::byte, 32>{
        reinterpret_cast<const std::byte*>(_identity.sk.data()), 32};
    auto pk = std::span<const std::byte, 32>{
        reinterpret_cast<const std::byte*>(_identity.pk.data()), 32};
    ...
    std::cout << "Session Router running"
              << " [" << (_config.is_relay ? "relay" : "client") << "]"
              << " port=" << _config.listen_port
              << " tun=" << (_tun.is_open() ? _config.tun_name : "none")
              << "\n";
...
void Node::maintain_paths() {
    ...
    while (static_cast<int>(_paths.size()) < _config.target_paths
           && _nodedb.has_min_rcs(6)) {
        build_path();
    }
}
...
void Node::handle_inbound_packet(std::span<const std::byte> packet) {
    _tun.write_packet(packet);
}
...
void Node::bootstrap() {
    ...
    // For now, just log
    if (_nodedb.size() < 6) {
        std::cout << "NodeDB has " << _nodedb.size()
                  << " RCs (need 6 minimum). Bootstrap required.\n";
    }
}
