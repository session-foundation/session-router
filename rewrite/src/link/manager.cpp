...
Manager::Manager(Endpoint& endpoint) : _endpoint{endpoint} {
    ...
    _endpoint.on_request([this](const RouterID& from, std::string_view method,
...
void Manager::on(const std::string& method, MessageHandler handler) {
    _handlers[method] = std::move(handler);
}

void Manager::on_datagram(DatagramHandler handler) {
    _endpoint.on_datagram(std::move(handler));
}

void Manager::send_datagram(const RouterID& to,
                            std::span<const std::byte> data) {
    _endpoint.send_datagram(to, data);
}

void Manager::send_request(const RouterID& to,
...
void Manager::connect(const RouterID& rid,
                      const std::string& addr, uint16_t port) {
    _endpoint.connect(rid, addr, port);
}

bool Manager::is_connected(const RouterID& to) const {
    return _endpoint.is_connected(to);
}

std::vector<RouterID> Manager::connected_peers() const {
    return _endpoint.connected_peers();
}
