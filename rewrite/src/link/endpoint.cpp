...
Endpoint::Endpoint(bool is_relay)
    : _is_relay{is_relay}, _impl{std::make_unique<Impl>(is_relay)} {}

Endpoint::~Endpoint() { close(); }

void Endpoint::listen(uint16_t port,
    ...
    _impl->tls_creds->request_client_keys(
        []([[maybe_unused]] std::span<const uint8_t> key,
           [[maybe_unused]] std::string_view alpn) -> bool {
            return true;
        });
    ...
    auto out_alpns = _is_relay
        ? quic::opt::outbound_alpns{"Session_Router_R"}
        : quic::opt::outbound_alpns{"Session_Router_C"};
            ...
            if (c.is_inbound()) {
                ...
                if (_bt_handler) {
                    ctrl->register_generic_handler([this, rid](quic::message msg) {
                        ...
                        _bt_handler(rid, ep_name, body_span,
                            [m = std::move(msg)](std::vector<std::byte> resp) mutable {
                                m.respond(std::string_view{
...
void Endpoint::connect(const RouterID& rid,
                       const std::string& addr, uint16_t port) {
    auto remote = quic::RemoteAddress{
        {reinterpret_cast<const unsigned char*>(rid.data()), 32},
        addr, port};

    auto conn = _impl->ep->connect(remote, _impl->tls_creds,
        quic::opt::keep_alive{10s},
        quic::opt::idle_timeout{60s});
...
void Endpoint::send_datagram(const RouterID& to,
...
void Endpoint::send_request(const RouterID& to,
    ...
    if (on_response) {
        it->second.control_stream->command(std::move(ep_name), payload,
            [cb = std::move(on_response)](quic::message msg) {
                if (msg) {
                    auto body = msg.body<std::byte>();
