...
namespace sr::session
{
    ...
    std::vector<std::byte> Session::encrypt(std::span<const std::byte> plaintext, TrafficType type) const
    {
    ...
    std::optional<std::vector<std::byte>> Session::decrypt(std::span<const std::byte> ciphertext) const
    {
    ...
    std::vector<std::byte> SessionInit::seal_for(
        const Ed25519PubKey& recipient, [[maybe_unused]] const Ed25519SecKey& our_sk) const
    {
    ...
    std::optional<SessionInit> SessionInit::unseal(
        std::span<const std::byte> sealed_data, const Ed25519PubKey& our_pk, const Ed25519SecKey& our_sk)
    {
        ...
        SessionInit si;
        size_t off = 0;
        std::memcpy(si.identity.data(), payload->data() + off, 32);
        off += 32;
        std::memcpy(si.x_pubkey.data(), payload->data() + off, 32);
        off += 32;
        std::memcpy(si.mlkem_pubkey.data(), payload->data() + off, MLKEM_PK_SIZE);
        off += MLKEM_PK_SIZE;
        std::memcpy(si.signature.data(), payload->data() + off, 64);
        off += 64;
        ...
        std::span<const std::byte> sealed_data, const Ed25519PubKey& our_pk, const Ed25519SecKey& our_sk)
    {
        ...
        SessionAccept sa;
        size_t off = 0;
        std::memcpy(sa.x_pubkey.data(), payload->data() + off, 32);
        off += 32;
        std::memcpy(sa.mlkem_ciphertext.data(), payload->data() + off, MLKEM_CT_SIZE);
        off += MLKEM_CT_SIZE;
        std::memcpy(sa.signature.data(), payload->data() + off, 64);
        off += 64;
