...
TEST_CASE("Manager registers and dispatches handlers", "[link][manager]") {
    ...
    mgr.on("test_method", [&](const RouterID&, std::span<const std::byte>,
...
TEST_CASE("Manager multiple handler registration", "[link][manager]") {
    ...
    int count = 0;
    mgr.on("method_a", [&](const RouterID&, std::span<const std::byte>,
                            std::function<void(std::vector<std::byte>)>) { ++count; });
    mgr.on("method_b", [&](const RouterID&, std::span<const std::byte>,
                            std::function<void(std::vector<std::byte>)>) { ++count; });
