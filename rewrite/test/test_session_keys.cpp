...
TEST_CASE("Session keys: initiator out != initiator in", "[crypto][session_keys]")
{
    ...
    auto keys = derive_session_keys(ix.pk, rx.pk, ix.sk, rx.pk, true, mlkem_ss, mlkem_pk, ie.pk, re.pk);
...
TEST_CASE("Session keys: swapped RouterIDs produce different keys", "[crypto][session_keys]")
{
    ...
    auto keys1 = derive_session_keys(ix.pk, rx.pk, ix.sk, rx.pk, true, mlkem_ss, mlkem_pk, ie.pk, re.pk);

    auto keys2 =
        derive_session_keys(ix.pk, rx.pk, ix.sk, rx.pk, true, mlkem_ss, mlkem_pk, re.pk, ie.pk);  // swapped RIDs
...
TEST_CASE("Session keys: different ML-KEM secret produces different keys", "[crypto][session_keys]")
{
    ...
    auto keys1 = derive_session_keys(ix.pk, rx.pk, ix.sk, rx.pk, true, ss1, mlkem_pk, ie.pk, re.pk);

    auto keys2 = derive_session_keys(ix.pk, rx.pk, ix.sk, rx.pk, true, ss2, mlkem_pk, ie.pk, re.pk);
